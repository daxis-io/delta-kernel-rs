...
/// like `/local/paths`, and even `../relative/paths` on native filesystem targets.
/// Targets without native filesystem path conversion require an absolute, non-file URL.
#[allow(unused)]
#[internal_api]
    ...
    let url = match uri_type {
        UriType::LocalPath(path) => {
            ...
            return Err(Error::invalid_table_location(format!(
                "Local filesystem paths are unavailable on this target: {path:?}"
            )));
    ...
    if let Ok(url) = Url::parse(&table_uri) {
        ...
        if url.scheme() == "file" {
            ...
            Err(Error::invalid_table_location(format!(
                "Local filesystem URLs are unavailable on this target: {table_uri}"
            )))
